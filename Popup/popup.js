...
создается попап, с анимацией от элемента и высотой окна попапа 150px.
2) $('селектор').makePopup('show', {'width': '100px','animationFrom':'center'}) - сразу показывается созданный попап,
с анимацией из центра и шириной окна 100px.
3) $('селектор').makePopup('close') - закрывается открытый попап.
4) $('селектор').makePopup() - по клику на елемент открывается попап, с анимацией из центра.
*/
